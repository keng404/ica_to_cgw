...
- ```--output_detect_mode``` : [OPTIONAL] argument to tell the script how to infer the ICA output files and directory. Two choces are either ```infererence``` or ```direct```. By default ```inference``` is the output_detect_mode used.
